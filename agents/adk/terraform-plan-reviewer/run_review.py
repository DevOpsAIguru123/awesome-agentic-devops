...
async def review(plan_text: str) -> dict:
    runner = InMemoryRunner(agent=root_agent, app_name="terraform_plan_reviewer")
    session = await runner.session_service.create_session(
        app_name="terraform_plan_reviewer", user_id="ci", session_id="ci-run"
    )
    final_response = None
    try:
...
if __name__ == "__main__":
    ...
    result = asyncio.run(review(plan_text))
